...
for index,a in enumerate(ordered):
 ...
 page=f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="{e(a['description'])}"><title>{e(a['title'])} · Fieldcraft</title><link rel="stylesheet" href="style.css"><link rel="alternate" type="application/rss+xml" title="Fieldcraft" href="feed.xml"><script type="module" src="app.js"></script></head><body><a class="skip" href="#main">Skip to content</a>{header}<main id="main" class="article-layout"><aside class="article-aside"><a href="./#library">← The collection</a><p class="eyebrow">FIELD NOTE {e(a['id'][3:])}</p><p>{e(a['subject'])}</p><hr><p><time datetime="{e(a['date'])}">{e(a['date'])}</time><span class="reading-time">{e(a['reading_minutes'])} MIN READ</span></p><span class="pill">{label}</span>{downloads}</aside><article><div class="eyebrow">{e(a['subject']).upper()}</div><h1>{e(a['title'])}</h1><p class="article-deck">{e(a['deck'])}</p>{body}<nav class="article-neighbors" aria-label="Adjacent field notes">{''.join(adjacent)}</nav><a class="back" href="./#library">← Back to Fieldcraft</a></article></main><footer><span>HEARTH & CODE · FIELDCRAFT</span><span>One technique. A working example. A visible result.</span><a href="feed.xml">RSS ↗</a></footer></body></html>'''
...
for a in ordered:
 item=ET.SubElement(channel,'item');url=f'https://hearthandcode.github.io/fieldcraft/{a["slug"]}.html'
